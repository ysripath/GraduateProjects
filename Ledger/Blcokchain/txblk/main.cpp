...
[P]rint : Output transactions that remain in the ledger and have not been output in a transaction block already.\n\
[H]elp : Command Summary\n\
[D]ump : Output transactions that have not been output in a transaction block already.\n\
[W]ipe : remove transactions that have not been output in a transaction block already.\n\
...
bool updateFileMap(string acName, string fileName)
{	
	...
	{
		if (verboseFlag)
			cout<<"File - "<<fileName<<" read success"<<endl;
		inFile.close();
	}
	else
		return false;
	...
	auto itr = fileMap.find(acName);
	// file already exists for account name
	if (itr != fileMap.end())
	{
		cout<<"Updating accountName - "<<acName<<" with new file - "<<fileName<<endl; 
        //fileMap.insert(pair<string, string>(acName, fileName));
        itr->second = fileName;
	}
	else
	{
...
;			}
		...
		case TRANSACTION: {
            ...
            head = isTransactionStringValid(transString, head, transMap, verboseFlag, signature);
			//transMap.erase(transMap.begin(), transMap.end());
			balanceMap.erase(balanceMap.begin(), balanceMap.end());
			updateMap(head);
			updateBalance(head);
...
// txblk		
		case OUTPUT: {
#ifdef DEBUG_LOG
			debugUtil("OUTPUT option selected");
#endif // DEBUG_LOG
            // Include valid transactions into a block
            int nTrans = 0; // To track number of transactions that gets included in current block;
            record* node = head;
            while (node) {
                if (!node->inBlock
                        && node->validRecord && !node->validated) {
                    if(checkSignature(node, fileMap, transMap, head, verboseFlag, true)) {
                        nTrans++;
                        node->inBlock = true;
                    }
                }
            ...
            // Output numbed of transactions to be included in the current block
            cout<<nTrans<<endl;
            node = head;
            while (node)
            {
                if (node->inBlock && !node->validated){
                    string transString = getRecord(node);
                    //cout<<node->transId<<"; "<<transString;
                    cout<<transString;
                    cout<<node->signature<<endl;
                    node->validated = true;
                }
                node = node->next;
            }
		...
		case CHECK: {
#ifdef DEBUG_LOG
			debugUtil("CHECK option selected");
#endif // DEBUG_LOG
			string transactionId;
			cin >> transactionId;

            auto it = transMap.find(transactionId);
            if (it == transMap.end())
            {
                cout<<"TransId: "<<transactionId<<" not found in ledger"<<endl;
                break;
            }
            ...
            while (node != NULL)
            {
                if (!node->transId.compare(transactionId)
                        && node->validRecord)
                {
                    if (node->validated)
                    {
                    ...
                    else if (checkSignature(node, fileMap, transMap, head, verboseFlag, false))
                    {
                        vFlag = true;
                    }
                    else
		...
		case READ: {
#ifdef DEBUG_LOG
			debugUtil("READ option selected");
#endif // DEBUG_LOG	
			if (INTERACTIVE_MODE) {
				cout<<"Input <account name> and <keyfilename>"<<endl;
			}
			string acName;
			string fileName;
			cin>>acName;
			cin>>fileName;
			if (updateFileMap(acName, fileName))
			{
				cout<<"Read <account name> and <keyfilename> sucess"<<endl;
			}
			else
			{
				cout<<"Read <account name> and <keyfilename> failed"<<endl;
			}
		} break;
