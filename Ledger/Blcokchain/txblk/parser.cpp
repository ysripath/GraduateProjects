	...
	if (fileHandle.is_open()) {
		...
		while (getline(fileHandle, line))
		{
			...
			if (validRecordFlag) {
				...
				if (head == NULL) {
                    head = new record(ltransactionId, lM, lvOut, lN, lop, signature);
					temp = head;
					firstRecordFlag = false;
				}
				else {
                    temp->next = new record(ltransactionId, lM, lvOut, lN, lop, signature);
					temp = temp->next;
				}
			}
			lM = -1;
...
// Utility function to check if the given transaction record is syntactically valid
record* isTransactionStringValid(string line, record* head, map<string, vector<pair<string, long int>>> transMap, bool verboseFlag, string signature)
{
